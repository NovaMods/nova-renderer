...
namespace nova {
    ...
    void key_callback(GLFWwindow *, int key, int scancode, int action, int mods) {
        nova_renderer::get_instance()->get_input_handler().queue_key_press_event({key,scancode,action,mods,1});
    }

    void key_character_callback(GLFWwindow *, unsigned int key) {
        nova_renderer::get_instance()->get_input_handler().queue_key_char_event({  key,1 });
    }

    void mouse_button_callback(GLFWwindow *, int button, int action, int mods) {
        nova_renderer::get_instance()->get_input_handler().queue_mouse_button_event({ button,action,mods ,1 });
    }

    void mouse_position_callback(GLFWwindow *, double xpos, double ypos) {
        nova_renderer::get_instance()->get_input_handler().queue_mouse_position_event({(int)xpos,nova_renderer::instance->get_game_window().get_size().y-(int) ypos,1 });
    }

    void mouse_scroll_callback(GLFWwindow *, double xoffset, double yoffset) {
        nova_renderer::get_instance()->get_input_handler().queue_mouse_scroll_event({ xoffset,yoffset,1 });
    }
