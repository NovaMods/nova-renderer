...
#define TEST_SETUP_LOGGER                                                                                                                  \
    [] {                                                                                                                                   \
        ...
        log.add_log_handler(nova::renderer::TRACE, [](auto msg) { std::cout << "TRACE: " << msg << std::endl; });                          \
        log.add_log_handler(nova::renderer::DEBUG, [](auto msg) { std::cout << "DEBUG: " << msg << std::endl; });                          \
        log.add_log_handler(nova::renderer::INFO, [](auto msg) { std::cout << "INFO: " << msg << std::endl; });                            \
        log.add_log_handler(nova::renderer::WARN, [](auto msg) { std::cerr << "WARN: " << msg << std::endl; });                            \
        log.add_log_handler(nova::renderer::ERROR, [error_log](auto msg) {                                                                 \
            std::cerr << "ERROR: " << msg << std::endl;                                                                                    \
            *error_log << "ERROR: " << msg << std::endl << std::flush;                                                                     \
        });                                                                                                                                \
        log.add_log_handler(nova::renderer::FATAL, [error_log](auto msg) {                                                                 \
            std::cerr << "FATAL: " << msg << std::endl;                                                                                    \
            *error_log << "FATAL: " << msg << std::endl << std::flush;                                                                     \
        });                                                                                                                                \
        log.add_log_handler(nova::renderer::MAX_LEVEL, [error_log](auto msg) {                                                             \
