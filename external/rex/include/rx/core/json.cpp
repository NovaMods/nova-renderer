...
namespace rx {
    ...
    json::shared::~shared() { m_allocator->deallocate(reinterpret_cast<rx_byte*>(m_root)); }

    json::json(memory::allocator* _allocator, const char* _contents, rx_size _length) : m_shared{nullptr}, m_value{nullptr} {
    ...
    json::json(memory::allocator* _allocator, const char* _contents) : json{_allocator, _contents, strlen(_contents)} {}

    json::json(const char* _contents) : json{&memory::g_system_allocator, _contents, strlen(_contents)} {}

    json::json(shared* _shared, struct json_value_s* _value) : m_shared{_shared->acquire()}, m_value{_value} {}

    optional<string> json::error() const {
        if(m_shared) {
            const auto code{static_cast<enum json_parse_error_e>(m_shared->m_error.error)};
            return string::format("%zu:%zu %s",
                                  m_shared->m_error.error_line_no,
                                  m_shared->m_error.error_row_no,
    ...
    rx_f32 json::as_float() const { return static_cast<rx_f32>(as_number()); }
