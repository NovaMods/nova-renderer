...
public class ItemBed extends Item
{
    ...
    public EnumActionResult onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand side, EnumFacing hitX, float hitY, float hitZ, float p_180614_9_)
    {
        ...
        else
        {
            ...
            if (playerIn.canPlayerEdit(pos, hitX, stack) && playerIn.canPlayerEdit(blockpos, hitX, stack))
            {
                boolean flag1 = worldIn.getBlockState(blockpos).getBlock().isReplaceable(worldIn, blockpos);
                boolean flag2 = flag || worldIn.isAirBlock(pos);
                boolean flag3 = flag1 || worldIn.isAirBlock(blockpos);

                if (flag2 && flag3 && worldIn.getBlockState(pos.down()).func_185896_q() && worldIn.getBlockState(blockpos.down()).func_185896_q())
                {
