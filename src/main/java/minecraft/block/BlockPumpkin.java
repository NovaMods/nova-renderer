...
public class BlockPumpkin extends BlockHorizontal
{
    ...
    protected BlockPattern getSnowmanPattern()
    {
        ...
        {
            this.snowmanPattern = FactoryBlockPattern.start().aisle(new String[] {"^", "#", "#"}).where('^', BlockWorldState.hasState(field_181085_Q)).where('#', BlockWorldState.hasState(BlockStateMatcher.forBlock(Blocks.snow))).build();
        }
    ...
    protected BlockPattern getGolemPattern()
    {
        ...
        {
            this.golemPattern = FactoryBlockPattern.start().aisle(new String[] {"~^~", "###", "~#~"}).where('^', BlockWorldState.hasState(field_181085_Q)).where('#', BlockWorldState.hasState(BlockStateMatcher.forBlock(Blocks.iron_block))).where('~', BlockWorldState.hasState(BlockStateMatcher.forBlock(Blocks.air))).build();
        }
