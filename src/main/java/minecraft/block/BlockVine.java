...
public class BlockVine extends Block
{
    ...
    private boolean recheckGrownSides(World worldIn, BlockPos pos, IBlockState state)
    {
        ...
        for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL)
        {
            ...
            if (((Boolean)state.getValue(propertybool)).booleanValue() && !this.func_185752_x(worldIn.getBlockState(pos.offset(enumfacing))))
            {
            ...
            if (worldIn.rand.nextInt(4) == 0)
            {
                ...
                if (enumfacing1 == EnumFacing.UP && pos.getY() < 255 && worldIn.isAirBlock(blockpos2))
                {
                    if (!flag)
                    {
                        ...
                        {
                            if (rand.nextBoolean() || !this.func_185752_x(worldIn.getBlockState(blockpos2.offset(enumfacing2))))
                            {
