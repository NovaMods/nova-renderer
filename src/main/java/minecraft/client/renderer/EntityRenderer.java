...
public class EntityRenderer implements IResourceManagerReloadListener
{
        ...
        if (entity != null)
        {
            if (this.mc.theWorld != null)
            {
                ...
                for (int j = 0; j < list.size(); ++j)
                {
                    ...
                    AxisAlignedBB axisalignedbb = entity1.getEntityBoundingBox().func_186662_g((double)entity1.getCollisionBorderSize());
    ...
    private void setupCameraTransform(float partialTicks, int pass)
    {
        ...
        // Camera zoom is never set to anything other than 1.0, so this isn't a problem at all
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime)
    {
        ...
        if (!this.mc.skipRenderWorld)
        {
            ...
            final int k1 = Mouse.getX() * scaledWidth / this.mc.displayWidth;
            final int l1 = scaledHeight - Mouse.getY() * scaledHeight / this.mc.displayHeight - 1;
            int framerateLimit = this.mc.gameSettings.limitFramerate;

            if (this.mc.theWorld != null)
            {
                this.mc.mcProfiler.startSection("level");
                int j = Math.min(Minecraft.getDebugFPS(), framerateLimit);
                j = Math.max(j, 60);
                long k = System.nanoTime() - nanoTime;
                long l = Math.max((long)(1000000000 / j / 4) - k, 0L);
                this.renderWorld(partialTicks, System.nanoTime() + l);

                if (this.mc.isSingleplayer() && this.field_184374_E < Minecraft.getSystemTime() - 1000L)
                {
                    this.field_184374_E = Minecraft.getSystemTime();

                    if (!this.mc.getIntegratedServer().func_184106_y())
                    {
                        this.func_184373_n();
                    }
                }
    ...
    {
        if (this.mc.renderGlobal.func_184382_g() > 10 && this.mc.renderGlobal.func_184384_n() && !this.mc.getIntegratedServer().func_184106_y())
        {
            BufferedImage bufferedimage = ScreenShotHelper.func_186719_a(this.mc.displayWidth, this.mc.displayHeight, this.mc.getFramebuffer());
            ...
            try
            {
                ...
                graphics.dispose();
                ImageIO.write(bufferedimage1, "png", (File)this.mc.getIntegratedServer().func_184109_z());
            }
            catch (IOException ioexception)
            {
    ...
    private void addRainParticles()
    {
        ...
        if (f != 0.0F)
        {
            ...
            if (j > 0 && this.random.nextInt(3) < this.rainSoundCounter++)
            {
                ...
                {
                    this.mc.theWorld.func_184134_a(d0, d1, d2, SoundEvents.weather_rain_above, SoundCategory.WEATHER, 0.1F, 0.5F, false);
                }
                else
                {
                    this.mc.theWorld.func_184134_a(d0, d1, d2, SoundEvents.weather_rain, SoundCategory.WEATHER, 0.2F, 1.0F, false);
                }
            }
        }
    }
    ...
    private void setupFog(int startCoords, float partialTicks)
    {
        ...
        else if (iblockstate.getMaterial() == Material.water)
        {
            ...
            if (entity instanceof EntityLivingBase)
            {
                ...
                {
                    GlStateManager.setFogDensity(0.1F - (float)EnchantmentHelper.func_185292_c((EntityLivingBase)entity) * 0.03F);
                }
            }
            else
            {
