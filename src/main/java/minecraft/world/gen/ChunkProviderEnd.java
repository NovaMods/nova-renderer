...
public class ChunkProviderEnd implements IChunkGenerator
{
    /** RNG. */
    private Random rand;
    protected static final IBlockState END_STONE_DEFAULT_STATE = Blocks.end_stone.getDefaultState();
    ...
    {
        this.worldObj = p_i46667_1_;
        this.mapFeaturesEnabled = p_i46667_2_;
        this.rand = new Random(p_i46667_3_);
                    ...
                    for (int l1 = 0; l1 < 4; ++l1)
                    {
                        ...
                        for (int i2 = 0; i2 < 8; ++i2)
                        {
                            ...
                            for (int j2 = 0; j2 < 8; ++j2)
                            {
                                ...
                                if (d15 > 0.0D)
                                {
                                    iblockstate = END_STONE_DEFAULT_STATE;
                                }
    ...
    public void func_185962_a(ChunkPrimer primer)
    {
        for (int i = 0; i < 16; ++i)
        {
            for (int j = 0; j < 16; ++j)
            {
                int k = 1;
                int l = -1;
                IBlockState iblockstate = END_STONE_DEFAULT_STATE;
                IBlockState iblockstate1 = END_STONE_DEFAULT_STATE;
                    ...
                    else if (iblockstate2.getBlock() == Blocks.stone)
                    {
                        if (l == -1)
                        {
                            if (k <= 0)
                            {
                                ...
                                iblockstate1 = END_STONE_DEFAULT_STATE;
                            }
    ...
    public Chunk provideChunk(int x, int z)
    {
        ...
        {
            this.field_185972_n.func_186125_a(this.worldObj, x, z, chunkprimer);
        }
