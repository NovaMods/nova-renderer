...
TEST(shader_tree_node, calculate_filters_simple) {
    auto node = nova::view::shader_tree_node("test", [](const auto& geom) {return geom.is_block;});
    ...
    nova::view::render_object not_a_block = {};
...
TEST(shader_tree_node, calculate_filters_one_child) {
    auto node = nova::view::shader_tree_node(
            "gbuffers_terrain",
            [](const auto &geom) {
                return geom.type == nova::view::geometry_type::block && !geom.is_transparent;
            },
            {
                    nova::view::shader_tree_node(
    ...
    nova::view::render_object damaged_block = {};
    damaged_block.type = nova::view::geometry_type::block;
    damaged_block.damage_level = 2;

    nova::view::render_object cloud = {};
    cloud.type = nova::view::geometry_type::cloud;
