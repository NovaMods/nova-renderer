...
int main(int argc, char **argv) {
    ...
    if(is_format_supported(physical_device, vk::Format::eR8G8B8Unorm, vk::FormatFeatureFlagBits::eTransferDstKHR)) {
        LOG(INFO) << "R8G8B8Unorm is supported as a transfer source";
    } else {
        LOG(ERROR) << "R8G8B8Unorm is not supported as a transfer source";
    }

    auto test_tex = nova::texture2D{};
    auto size = glm::u32vec2{4096, 2048};
    test_tex.set_data(nullptr, size, vk::Format::eR8G8B8Unorm);
