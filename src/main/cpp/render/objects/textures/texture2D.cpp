...
namespace nova {
    ...
    void texture2D::set_data(std::vector<float> &pixel_data, glm::ivec2 &dimensions, GLenum format, GLenum type, GLenum internal_format) {
        GLint previous_texture;
        glGetIntegerv(GL_TEXTURE_BINDING_2D, &previous_texture);
        glBindTexture(GL_TEXTURE_2D, gl_name);
        glTexImage2D(GL_TEXTURE_2D, 0, internal_format, dimensions[0], dimensions[1], 0, format, type, pixel_data.data());
