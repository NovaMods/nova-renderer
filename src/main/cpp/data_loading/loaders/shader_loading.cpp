...
namespace nova {
    namespace model {
        ...
        nova::view::shaderpack load_sources_from_folder(
            ...
            // shaderpack
            nlohmann::json shaders_json = shaders_json_file.is_open() ? load_json_from_stream(shaders_json_file) : nlohmann::json();
            ...
            for(auto& shader : shaders) {
                try {
                    ...
                    sources.push_back(full_shader_source);
            ...
            for(auto def : sources) {
                if(def.fallback) {
                    ...
                    for(auto test : sources) {
                        if(test.name == *def.fallback) {
                            found_fallback = true;
                        }
                    }

                    if(!found_fallback) {
                        LOG(WARN) << "Could not find fallback shader " << def.fallback << " for shader " << def.name << ".";
                    }
                } else {
                    LOG(WARN) << "No fallback specified for shader " << def.name << ". If you forget that shader, its geometry won't render";
                }
            }
