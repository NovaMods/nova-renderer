...
namespace nova {
    ...
    uniform_buffer::uniform_buffer(std::string name, const VmaAllocator allocator, VkBufferCreateInfo create_info, uint64_t min_alloc_size, bool mapped) : name(std::move(name)), min_alloc_size(min_alloc_size), allocator(allocator) {
        ...
        auto buffer_create_result = vmaCreateBuffer(allocator, reinterpret_cast<const VkBufferCreateInfo*>(&create_info), &alloc_create, reinterpret_cast<VkBuffer*>(&buffer), &allocation, &allocation_info);
