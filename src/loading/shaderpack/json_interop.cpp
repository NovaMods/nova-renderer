...
namespace nova::renderer::shaderpack {
    ...
    void from_json(const nlohmann::json& j, render_pass_create_info_t& pass) {
        ...
        pass.texture_outputs = get_json_array<texture_attachment_info_t>(j, "textureOutputs");
        pass.depth_texture = get_json_value<texture_attachment_info_t>(j, "depthTexture");
    ...
    void from_json(const nlohmann::json& j, pipeline_create_info_t& pipeline) {
        ...
        pipeline.vertex_fields = get_json_array<vertex_field_data_t>(j, "vertexFields");
        ...
        if(geometry_shader_name) {
            pipeline.geometry_shader = std::make_optional<shader_source_t>();
        ...
        if(tess_control_shader_name) {
            pipeline.tessellation_control_shader = std::make_optional<shader_source_t>();
        ...
        if(tess_eval_shader_name) {
            pipeline.tessellation_evaluation_shader = std::make_optional<shader_source_t>();
        ...
        if(fragment_shader_name) {
            pipeline.fragment_shader = std::make_optional<shader_source_t>();
    ...
    void from_json(const nlohmann::json& j, material_data_t& mat) {
        ...
        mat.passes = get_json_array<material_pass_t>(j, "passes");
        mat.geometry_filter = get_json_value<std::string>(j, "filter").value();

        for(material_pass_t& pass : mat.passes) {
    ...
    void from_json(const nlohmann::json& j, std::vector<render_pass_create_info_t>& passes) {
        for(const auto& node : j) {
            passes.push_back(node.get<render_pass_create_info_t>());
        }
    }
