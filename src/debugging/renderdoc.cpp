...
namespace nova::renderer {
    result<RENDERDOC_API_1_3_0*> load_renderdoc(const std::string& renderdoc_dll_path) {
        ...
        if(!renderdoc_dll) {
            ...
            return error_result<RENDERDOC_API_1_3_0*>(fmt("Could not load RenderDoc. Error: {:s}"), error);
        }
        ...
        if(!get_api) {
            const std::string error = get_last_windows_error();
            return error_result<RENDERDOC_API_1_3_0*>(fmt("Could not load RenderDoc DLL. Error: {:s}"), error);
        }
        ...
        if(renderdoc_so == nullptr) {
            ...
            if(renderdoc_so == nullptr) {
                return error_result<RENDERDOC_API_1_3_0*>("Could not load RenderDoc. Error: {:s}", dlerror());
            }
        }

        const auto get_api = reinterpret_cast<pRENDERDOC_GetAPI>(dlsym(renderdoc_so, "RENDERDOC_GetAPI"));
        if(get_api == nullptr) {
            return error_result<RENDERDOC_API_1_3_0*>("Could not find the RenderDoc API loading function. Error: {:s}", dlerror());
        }
#endif
        ...
        if(ret != 1) {
            ...
            return error_result<RENDERDOC_API_1_3_0*>(fmt("Could not load RenderDoc API. Error code {:d}"), ret);
        }
