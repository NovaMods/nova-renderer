...
public abstract class BlockRedstoneDiode extends BlockHorizontal
{
    ...
    {
        return worldIn.getBlockState(pos.down()).func_185896_q() ? super.canPlaceBlockAt(worldIn, pos) : false;
    }
    ...
    {
        return worldIn.getWeakPower(pos, state, side);
    }

    public int getWeakPower(IBlockState worldIn, IBlockAccess pos, BlockPos state, EnumFacing side)
    {
        return !this.isPowered(worldIn) ? 0 : (worldIn.getValue(FACING) == side ? this.getActiveSignal(pos, state, worldIn) : 0);
    }
