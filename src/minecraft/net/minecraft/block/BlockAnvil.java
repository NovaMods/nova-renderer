...
public class BlockAnvil extends BlockFalling
{
    ...
    public IBlockState onBlockPlaced(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer)
    {
        ...
        catch (IllegalArgumentException var11)
        {
            if (!worldIn.isRemote)
            {
                field_185762_e.warn(String.format("Invalid damage property for anvil at %s. Found %d, must be in [0, 1, 2]", new Object[] {pos, Integer.valueOf(meta >> 2)}));
    ...
    {
        return state.getBlock() != this ? state : state.withProperty(FACING, rot.func_185831_a((EnumFacing)state.getValue(FACING)));
    }
